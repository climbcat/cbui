...
void TestLayoutPanels() {
    ...
    while (loop->GameLoopRunning()) {
        ...
        w0->flg_features |= WF_EXPAND_VERTICAL;
        w0->flg_features |= WF_EXPAND_HORIZONTAL;

        {
            // wrapper
            Widget *w1 = UI_LayoutHorizontal();
            w1->flg_features |= WF_EXPAND_VERTICAL;
            w1->flg_features |= WF_EXPAND_HORIZONTAL;

            {
                // list view
                Widget *l_nodes = UI_LayoutVertical();
                   l_nodes->flg_features |= WF_DRAW_BACKGROUND_AND_BORDER;
                l_nodes->col_bckgrnd = ColorRed();
                l_nodes->col_border = ColorGray(0.9f);
                l_nodes->sz_border = 5;
                l_nodes->flg_features |= WF_EXPAND_VERTICAL;
                l_nodes->w = 740;

                {
                    ...
                    itm_1->flg_features |= WF_EXPAND_HORIZONTAL;
                    itm_1->h = 20;
                }
                {
                    Widget *itm_1;
...

            {
                ...
                l_btns->flg_features |= WF_DRAW_BACKGROUND_AND_BORDER;
                l_btns->col_bckgrnd = ColorBlue();
                l_btns->col_border = ColorGray(0.7f);
                l_btns->sz_border = 5;
                l_btns->flg_features |= WF_EXPAND_HORIZONTAL;
                l_btns->flg_features |= WF_EXPAND_VERTICAL;
        ...
        l_status->flg_features |= WF_DRAW_BACKGROUND_AND_BORDER;
        l_status->col_border = ColorGray(0.5f);
        l_status->sz_border = 5;
        l_status->col_bckgrnd = ColorGreen();
        l_status->h = 50;
        l_status->flg_features |= WF_EXPAND_HORIZONTAL;
